            ...
            <div class="col-12">
              <ul class="list-group product-list">
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center active-list">
                  ...
                  <a href="#" class="btn btn-icons btn-rounded btn-inverse-outline-primary font-weight-bold btn-pilih" data-id-barang="{{ $product->id }}" role="button"><i class="mdi mdi-chevron-right"></i></a>
                </li>
                @endif
                @else
                <li class="list-group-item d-flex justify-content-between align-items-center active-list">
          ...
          <div class="row">
            ...
            <div class="col-12">
              <table class="table-receipt">
                ...
                <tr>
                  <td>
                    <span class="d-block little-td">{{(auth()->user()->role == "pelanggan") ? "User" : "Kasir"}}</span>
                ...
                <div class="col-12 d-flex justify-content-between align-items-center transaction-header">
                  ...
                  <div class="btn-group mt-h">
                    <button class="btn btn-search" data-toggle="modal" data-target="#tableModal" type="button">
                      Pilih Jasa <i class="mdi mdi-magnify"></i>
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div class="row">
            <div class="col-12 payment-1">
              <table class="table-payment-1">
                ...
                <tr>
                  <td class="text-left">{{(auth()->user()->role == "pelanggan") ? "User" : "Kasir"}}</td>
...
$(document).on('click', '.btn-pilih', function(e){
  e.preventDefault();
  var kode_barang = $(this).data('id-barang');
  $.ajax({
    url: "{{ url('/transaction/product') }}/" + kode_barang,
    method: "GET",
    success:function(response){
      var check = $('.kode-barang-td:contains('+ response.product.id +')').length;
      if(check == 0){
        tambahData(response.product.id, response.product.nama_barang, response.product.harga, response.product.stok, response.status);
      }else{
        swal(
            "",
